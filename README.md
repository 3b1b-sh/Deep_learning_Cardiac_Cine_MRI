# Deep Learing Cardiac Cine MRI Segmentation
...
python unet.py 
# or
pyhon attention_unet.py
```

4. Notes
In the folder of model, there are some pre-trained models. You can use them directly. Or you can train the model by yourself.
...
We use U-Net for the segmentation task and the basical structure of our model is as following:
![](img/DoubleConv.png)

What slightly different from the model above, the output channel of our model is 3 and the size of image is $256\times256$ .
...
Our basical training setting is as following
...
#### 4. Results
Basically, we use the Dice coefficient to evaluate the performance of our model.

Dice coefficient is a measure of the similarity between two probability distributions. It ranges from 0 to 1, where 0 indicates no similarity and 1 indicates perfect similarity.

**The calculation of the Dice coefficient is as following: $ \text{Dice Coefficient} = \frac{2 \times |X \cap Y|}{|X| + |Y|} $**



Following is the Dice coefficients of LV, RV and MYO for all testing slices
|                        | RV     | MYO    | LV     |
| ---------------------- | ------ | ------ | ------ |
...
Training loss and evaluation loss is as following:
...
2. The average segmentation performance for the Myocardium (MYO) is the worst of three, with a mean Dice coefficient of 0.8743, slightly lower than the LV. However, the standard deviation of 0.0156 is significantly lower than the LV, suggesting that the segmentation results for the MYO are more stable and consistent compared to the LV.
...
To improve the segmentation performance, we consider the some modifications to the U-Net architecture:
...
Following is the comparation of the segmentation performance (Dice coefficient) of the two networks.
...
Training loss and evaluation loss of U-Net without short-cut connection is as following:
...
2. On the other hand, long skip connections are used to pass features from the encoder path to the decoder path in order to recover spatial information lost during downsampling. Short skip connections appear to stabilize gradient updates in deep architectures. Finally, skip connections enable feature reusability and stabilize training and convergence.
...
Training loss and evaluation loss of U-Net with data argumentation is as following:
...
#### 3. Analysis
We compare the training loss and evaluation loss between original U-Net, U-Net without data argumentation and U-Net with data argumentation.
...
We comprehensively consider the average test loss and it's standard deviation and the evalution curve of training step. We consider the U-Net model with data argumentation as the best model.
...
Following is the segmentation accuracy of the two model with cross-entropy loss and soft Dice loss
...
Training loss and evaluation loss of U-Net with soft Dice loss is as following:
| Training loss                                             |
| --------------------------------------------------------- |
| ![](img/12.png)  |
| **Evaluation loss**                                       |
...
3. **Implications**:
   ...
   - **Soft Dice Loss**: Although it shows lower accuracy, the soft Dice loss is designed to handle class imbalance by focusing on overlap between predicted and ground truth regions. This might explain why it has higher variability, as it tries to optimize for different aspects of the segmentation.

### Strategy 4: Attention U-Net
As mentioned in the previous strategies, through comprehensively considering the average test loss and it's standard deviation and the evalution curve of training step, we regard the U-Net model with data argumentation as the best model.

In this part, we try to introduce the Attention U-Net model to see if attention mechanism can improve the performance of the model.

Now, our model is Attention U-Net with data argumentation.

the basical structure of our model is as following:
![](img/15.png)
![](img/16.png)
...
Compared to the regular U-Net decoder, the Attention U-Net incorporates an attention mechanism that gates the features from the encoder before concatenating them with the decoder features. This attention gating process enhances the feature maps by including importance information from different spatial positions, enabling the model to focus more on specific target regions.
#### 1. Results
